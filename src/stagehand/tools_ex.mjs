...
export function registerStagehandTools({ mcpServer, getSession, safeGetPage, generatePlaywrightScript, asset }) {
  ...
  mcpServer.addTool({
    ...
    description: '创建新页面并设为活动页。可选：创建后立即导航到指定 URL。适合开始新任务或隔离操作上下文，返回新页索引与总页数。',
    parameters: z.object({
  ...
  mcpServer.addTool({
    ...
    description: '列出当前会话的所有页面索引，并标注活动页索引。默认单页模式下通常仅有一个索引；启用多页后用于调试与页面切换场景。',
    parameters: z.object({}),
  ...
  mcpServer.addTool({
    ...
    description: ENABLE_MULTI_PAGE
      ? '设置当前会话的活动页面索引（多页模式）。'
      : '设置当前会话的活动页面索引（建议仅在多页模式开启时使用）。未开启多页时请优先在活动页内操作或使用 stagehand_goto 导航。',
    parameters: z.object({
      pageIndex: z.number().int().nonnegative().describe('要设为活动的页面索引（从 0 开始）'),
    }),
  ...
  mcpServer.addTool({
    ...
    parameters: (ENABLE_MULTI_PAGE
      ? z.object({ url: z.string().url().describe('要导航的 URL'), pageIndex: z.number().int().nonnegative().optional().describe('可选：目标页面索引') })
      : z.object({ url: z.string().url().describe('要导航的 URL') })
    ),
  ...
  mcpServer.addTool({
    name: 'stagehand_act',
    description: '页面操作：每次仅执行一个具体操作（最佳实践）。使用描述性语言与正确动词（点击/输入/选择等），避免颜色等视觉属性。可传确定性动作对象跳过推理；敏感数据请用 variables。建议先使用 stagehand_observe 发现元素再行动。',
    parameters: z.object({
  ...
  mcpServer.addTool({
    ...
    parameters: (ENABLE_MODEL_OVERRIDE
      ? z.object({
          ...
          options: z.object({
              model: z.string().optional().describe('进阶：为本次观察指定模型。'),
              timeout: z.number().optional().describe('进阶：覆盖超时（毫秒）。'),
              selector: z.string().optional().describe('进阶：覆盖选择器。')
          }).optional().describe('兼容旧形态的配置对象。')
        })
      : z.object({
          instruction: z.string().describe('自然语言描述要发现的元素或操作；为空则返回所有可交互元素。'),
          selector: z.string().optional().describe('可选：限制在该选择器范围内观察（XPath/CSS）。'),
  ...
  mcpServer.addTool({
    ...
    description: '页面提取：根据指令与可选 Zod 模式提取结构化数据。使用描述性字段、正确类型，并为 URL 使用 z.string().url()（最佳实践）。\n高级参数：模型覆盖通过 STAGEHAND_ENABLE_MODEL_OVERRIDE=1 开启。',
    parameters: (ENABLE_MODEL_OVERRIDE
      ? z.object({
